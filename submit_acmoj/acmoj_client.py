...
# -*- coding: utf-8 -*-
"""
ACMOJ API Client Command Line Tool - C++ File Submission Version v2.3

Usage Examples:
1. Submit C++ source file:
   python3 acmoj_client.py --token ${ACMOJ_TOKEN} submit --problem-id ${ACMOJ_PROBLEM_ID} --language cpp --code-file path/to/file.cpp
   The returned result contains submission_id information, please save it for subsequent status queries

2. Submit Git repository URL (recommended for Git-evaluated problems):
   python3 acmoj_client.py --token ${ACMOJ_TOKEN} submit-git --problem-id ${ACMOJ_PROBLEM_ID} --git-url https://github.com/user/repo
...
class ACMOJClient:
    ...
    def _make_request(self, method: str, endpoint: str, data: Dict[str, Any] = None,
        ...
        try:
            ...
            try:
                print(f"Response text: {response.text}")  # type: ignore[name-defined]
            except Exception:
                pass
            return None
...
def main():
    ...
    # Submit C/C++/etc. source text
    submit_parser = subparsers.add_parser("submit", help="Submit source code text")
